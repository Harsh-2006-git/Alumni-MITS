...
export default function JobsPage({ isDarkMode, toggleTheme }) {
  ...
  const fetchJobs = async () => {
    try {
      ...
      if (data.success) {
        // Filter jobs where isAutoPosted is false (alumni-posted jobs)
        const alumniPostedJobs = (data.data || []).filter(
          (job) => job.isAutoPosted === false
        );
        setJobs(alumniPostedJobs);
        showToast(
          `Loaded ${alumniPostedJobs.length} alumni-posted jobs successfully`,
          "success"
        );
      } else {
